...
CREATE INDEX idx_pattern_type ON memory_patterns(pattern_type);
CREATE INDEX idx_context ON memory_patterns(context);
CREATE INDEX idx_quality_score ON memory_patterns(quality_score DESC);
CREATE INDEX idx_embedding ON memory_patterns USING ivfflat (embedding vector_cosine_ops);
...
CREATE INDEX idx_session_context ON sessions(context);
...
CREATE INDEX idx_interaction_session ON interactions(session_id, sequence_num);
CREATE INDEX idx_prompt_embedding ON interactions USING ivfflat (prompt_embedding vector_cosine_ops);
...
-- Create index for context_bridges
CREATE INDEX idx_bridge_contexts ON context_bridges(source_context, target_context);
...
CREATE INDEX idx_learning_session ON learning_events(session_id);
CREATE INDEX idx_learning_type ON learning_events(event_type);
CREATE INDEX idx_impact_score ON learning_events(impact_score DESC);
...
-- Create index for pattern_relationships
CREATE INDEX idx_pattern_relations ON pattern_relationships(pattern_a, pattern_b);
...
-- Create indexes for global_insights
CREATE INDEX idx_insight_type ON global_insights(insight_type);
CREATE INDEX idx_confidence ON global_insights(confidence DESC);
...
BEGIN
    ...
    WHERE (context_filter IS NULL OR mp.context = context_filter)
...
BEGIN
    SELECT
        ...
        usage_count,
        (SELECT COUNT(DISTINCT target_context) FROM context_bridges WHERE pattern_id = p_pattern_id)
